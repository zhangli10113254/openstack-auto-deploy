...
class KeystoneInstallTask(OpenstackServiceInstallTask):
    ...
    def doExecute(self):
        ...
        utils.local_call('openstack service create --name keystone --description "OpenStack Identity" identity', env = self.env)
        utils.local_call("openstack endpoint create --region RegionOne identity public http://%s:5000/v3" % self.host['ip'], env = self.env)
        utils.local_call("openstack endpoint create --region RegionOne identity internal http://%s:5000/v3" % self.host['ip'], env = self.env)
        utils.local_call("openstack endpoint create --region RegionOne identity admin http://%s:5000/v3" % self.host['ip'], env = self.env)    
        
        utils.local_call('openstack domain create --description "Default Domain" default', env = self.env)
        utils.local_call("openstack project create --domain default --description 'Admin Project' admin", env = self.env)
...
class NovaInstallTask(OpenstackServiceInstallTask):
    ...
    def doExecute(self):
        ...
        utils.local_call("openstack endpoint create --region RegionOne compute public http://%s:8774/v2.1/%%\(tenant_id\)s" % self.host['ip'], env = self.env)
        utils.local_call("openstack endpoint create --region RegionOne compute internal http://%s:8774/v2.1/%%\(tenant_id\)s" % self.host['ip'], env = self.env)
        utils.local_call("openstack endpoint create --region RegionOne compute admin http://%s:8774/v2.1/%%\(tenant_id\)s" % self.host['ip'], env = self.env)
        
        commands = []
        commands.append("mysql -uroot -e 'create database nova'")
        ...
        commands.append('yum install -y openstack-nova-api openstack-nova-conductor openstack-nova-console openstack-nova-novncproxy openstack-nova-scheduler')
        ...
        commands.append("openstack-config --set /etc/nova/nova.conf DEFAULT rpc_backend rabbit")
        
        commands.append("openstack-config --set /etc/nova/nova.conf oslo_messaging_rabbit rabbit_host %s" % self.host['ip'])
        commands.append("openstack-config --set /etc/nova/nova.conf oslo_messaging_rabbit rabbit_userid openstack")
        commands.append("openstack-config --set /etc/nova/nova.conf oslo_messaging_rabbit rabbit_password openstack")
        ...
        commands.append("openstack-config --set /etc/nova/nova.conf vnc novncproxy_host $my_ip")
...
class NeutronInstallTask(OpenstackServiceInstallTask):
    ...
    def doExecute(self):
        ...
        commands.append("openstack-config --set /etc/neutron/neutron.conf DEFAULT rpc_backend rabbit")
        commands.append("openstack-config --set /etc/neutron/neutron.conf oslo_messaging_rabbit rabbit_host %s" % self.host['ip'])
        commands.append("openstack-config --set /etc/neutron/neutron.conf oslo_messaging_rabbit rabbit_userid openstack")
        commands.append("openstack-config --set /etc/neutron/neutron.conf oslo_messaging_rabbit rabbit_password openstack")
        ...
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/ml2_conf.ini ml2 type_drivers flat,vlan,vxlan,gre")
        ...
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/ml2_conf.ini ml2_type_flat flat_networks external")
        ...
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/ml2_conf.ini securitygroup enable_security_group true")
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/ml2_conf.ini securitygroup firewall_driver iptables_hybrid")
        
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/openvswitch_agent.ini securitygroup enable_ipset true")
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/openvswitch_agent.ini securitygroup enable_security_group true")
        ...
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/openvswitch_agent.ini ovs integration_bridge br-int")
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/openvswitch_agent.ini ovs tunnel_bridge br-tun")
        commands.append("openstack-config --set /etc/neutron/plugins/ml2/openvswitch_agent.ini ovs bridge_mappings external:br-ex")
        ...
        commands.append("ovs-vsctl add-br br-ex")
        commands.append("ovs-vsctl add-port br-ex %s" % self.provider_interface)
    ...
    def process_params(self):
        ...
        self.provider_interface = self.kwargs["controller"]["service_properties"]["neutron"]['provider_interface']
...
class CinderInstallTask(OpenstackServiceInstallTask):
    ...
    def doExecute(self):
        ...
        commands.append("openstack-config --set /etc/cinder/cinder.conf DEFAULT rpc_backend rabbit")
        commands.append("openstack-config --set /etc/cinder/cinder.conf oslo_messaging_rabbit rabbit_host %s" % self.host['ip'])
        commands.append("openstack-config --set /etc/cinder/cinder.conf oslo_messaging_rabbit rabbit_userid openstack")
        commands.append("openstack-config --set /etc/cinder/cinder.conf oslo_messaging_rabbit rabbit_password openstack")
...
class ComputeInstallTask(OpenstackServiceInstallTask):
    ...
    def doExecute(self):
        ...
        commands.append('sed -i -e "s#\#listen_tls *= *0#listen_tls = 0#g" /etc/libvirt/libvirtd.conf')
        commands.append('sed -i -e "s#\#listen_tcp *= *1#listen_tcp = 1#g" /etc/libvirt/libvirtd.conf')
        commands.append('sed -i -e "s#\#auth_tcp *= *\"sasl\"#auth_tcp = \"none\"#g" /etc/libvirt/libvirtd.conf')
        commands.append('sed -i -e "s#\#LIBVIRTD_ARGS *= *\"--listen\"#LIBVIRTD_ARGS=\"--listen\"#g" /etc/sysconfig/libvirtd')
        ...
        #neutron
        commands.append("yum install -y openstack-neutron openstack-neutron-ml2 openstack-neutron-openvswitch ebtables ipset")
